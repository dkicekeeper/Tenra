...
class DepositsViewModel {
    ...
    /// Uses the per-account index when reachable — O(M) walk instead of O(N_tx).
    func reconcileDepositInterest(for accountId: String, allTransactions: [Transaction], onTransactionCreated: @escaping (Transaction) -> Void) {
        ...
        let scoped = accountsViewModel.transactionStore?.transactionsByAccount[accountId] ?? allTransactions
        DepositInterestService.reconcileDepositInterest(
            account: &account,
            allTransactions: scoped,
            onTransactionCreated: onTransactionCreated
    ...
    func recalculateInterest(for accountId: String, transactionStore: TransactionStore) async throws {
        ...
        // O(M) via per-account index; deposit interest tx is always on the deposit
        // account so this is the right bucket.
        let postedInterest = (transactionStore.transactionsByAccount[accountId] ?? [])
            .filter {
