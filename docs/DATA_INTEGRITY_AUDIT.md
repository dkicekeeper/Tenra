...
- **Next (Phase 5 remainder, large):** unify `InsightsService` internal metric computations (`PreAggregatedData.build`, `computePeriodDataPoints`, `resolveAmount`) — H-5 (future-date handling inconsistent across granularities), H-6 (eligible-type mismatch vs store), H-7 (cold-FX `convertedAmount`-as-base with no reconcile). Plus M-1 (deposit principalDelta), M-9 (single FX-version), M-4, H-10.
