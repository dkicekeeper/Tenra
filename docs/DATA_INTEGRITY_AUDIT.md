...
- **Next:** Phase 3 (loan/deposit debt single-source: C-3, M-1, M-4 — needs product confirmation on deriving loan balance from `remainingPrincipal`), Phase 5 (Insights↔store unification: C-6/H-5/H-6/H-7), remaining Phase 6 (H-10 import partial-write atomicity), M-9 (unify FX-version signal).
