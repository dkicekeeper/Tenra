...
- **Next:** Phase 3 deposit side (M-1 unify `DepositInterestService.principalDelta`; M-2 run deposit reconcile on launch/day-change), Phase 5 (Insights↔store unification: C-6/H-5/H-6/H-7), remaining Phase 6 (H-10 import partial-write atomicity), M-9 (unify FX-version signal), M-4 (recalculateAfterLinking actual amounts).
