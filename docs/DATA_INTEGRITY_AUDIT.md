...
- **2026-05-25 — M-3 INVESTIGATED → CONFIRMED can occur → DEFERRED (needs product decision; no half-measure).**
  ...
  - **Decision needed (DO NOT implement a half-measure):** product/eng must choose the future-dated-loan-payment semantics: **(A)** Block future dates on the loan payment/early-repayment forms (add `in: ...Date()` to the picker) — simplest, matches "realized" intent, but inconsistent with regular tx which allow future dates. **(B)** Defer the `remainingPrincipal` reduction until the payment date matures: stop mutating `remainingPrincipal` synchronously for future-dated payments and have the maturation trigger (`recalculateLedgerIfDayChanged`) recompute `remainingPrincipal` from REALIZED (date ≤ today) loan-payment/early-repayment transactions when their date arrives. This is the "correct" model but a substantial change to loan-debt accounting (principal must become a derived-from-realized-tx quantity, like balance, rather than an eagerly-mutated cached field) and touches every loan write path. Per the refactor's safety rule, left UNIMPLEMENTED pending the decision.
