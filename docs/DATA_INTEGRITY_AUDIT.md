...
| H-6 | Insights | ✅ DONE (budget path). Store budget "spent" now reads `CategoryAggregate.expenseAmount` (expense-only, v10) instead of the all-types `totalAmount` — matches the Insights expense-only path. | TransactionStore+CategoryIndex.swift, CategoryBudgetService.readBucket |
| H-7 | Currency | ✅ DONE. Cold-FX-cache fallback no longer blends `convertedAmount` (account currency) as base. `resolveAmountToBase` skips the unconvertible cross-currency tx (returns 0) + flags `PreAggregatedData.fxStale`; self-heals on the M-9 FX-version bump. | InsightsService.swift `resolveAmountToBase` |
...
| H-10 | Persistence | ✅ DONE (refined). Aggregates were ALREADY safe (finishImport re-throw precedes flush*; schedulers isImporting-gated). The real partial-write was BALANCES: coordinator swallowed the finishImport error then persisted balances from in-memory-only tx. Now gated via `mayPersistBalances(transactionsPersisted:)`. | CSVImportCoordinator.swift |
...
- **2026-05-25 — M-1 CHARACTERIZED, math NOT changed (high-risk, DEFERRED by design).** Added `DepositPrincipalDeltaCharacterizationTests` (14 tests) pinning the CURRENT behavior of `DepositInterestService.principalDelta`, and documented its three precise divergences from `BalanceCalculationEngine.contribution` in the `principalDelta` doc-comment + `docs/domains/deposits.md`: (1) eligible types & `convertedAmount` vs `targetAmount` for cross-currency inflows; (2) `.depositInterestAccrual` is capitalization-gated in `principalDelta` but not in `contribution`; (3) `.internalTransfer` source leg uses raw `-amount` here vs `-(convertedAmount ?? amount)` in `contribution` (asymmetric with its own target leg). Because these are genuine, interest-accrual-critical semantic differences (not provably behavior-identical), per the refactor's safety rule the math was left UNCHANGED and a clearly-marked TODO added. **Decision needed:** product/eng must confirm intended semantics for the three points before any unification.
- **2026-05-25 — H-10 ANALYZED + balance path FIXED.** Traced the import persist sequencing. **Aggregates are already safe**: `finishImport()` saves tx inside a do/catch that RE-THROWS on failure (TransactionStore.swift ~635) *before* `isImporting = false` and the `flushAggregatePersist()` / `flushAccountAggregatePersist()` calls — so an aggregate flush never runs when tx-save fails; and the debounced schedulers are `!isImporting`-gated throughout the import. `rebuildAggregateCacheAfterImport()` only invalidates an in-memory cache (no persist). **Balances are NOT safe**, though: `CSVImportCoordinator` SWALLOWS the `finishImport` error (records `stats.persistenceError`) and then runs `balanceCoordinator.setInitialBalance` + `recalculateAll`, both of which call `persistBalance(s)` (NOT gated by `isImporting`) → a balance reflecting in-memory-only tx is written to disk → next launch overcounts. Fix: the coordinator now tracks `transactionsPersisted` and gates the balance recalc/persist block on `CSVImportCoordinator.mayPersistBalances(transactionsPersisted:)`; on failure it logs and skips (the user is already warned via `persistenceError`). Test `CSVImportDurabilityTests` pins the rule (a full failing-CoreData-save integration test was impractical given the coordinator's viewmodel/stack dependencies; the substantive change is the control-flow guard, verified by analysis + build + full suite). Full suite green.
- **2026-05-25 — H-6 / C-6 residual DONE.** Budget "spent" is now EXPENSE-only on BOTH paths. `CategoryAggregate` gained an `expenseAmount` field (base currency) maintained in `applyAggregateDelta`/`patchBucket` (only `.expense` tx contribute); `CategoryBudgetService.readBucket` now reads `expenseAmount` instead of `totalAmount` (which sums all aggregatable types). So an income/loan/deposit tx tagged to a *budgeted expense category* no longer inflates the store-path "spent", which now matches the Insights legacy path (`calculateSpentLegacy`, already expense-only). Persisted via additive CoreData **schema bump v9→v10** (`expenseAmount: Double`, `expenseAmountSet: Bool`; legacy rows fall back to `totalAmount` so no warm-start regression). `CategoryDetailView` totals still read `totalAmount` (different consumer, H-3 future-exclusion already done) — intentionally out of scope. RED test `CategoryBudgetExpenseScopedTests` pins store-path == Insights-path for a mixed expense+income category, plus loan-tagged exclusion. Full suite green.
- **2026-05-25 — H-7 residual DONE.** The Insights amount resolvers no longer blend mismatched units into a base-currency total. `resolveAmountStatic` / `resolveAmount` now delegate to a single `InsightsService.resolveAmountToBase(_:baseCurrency:) -> ResolvedAmount` (mirrors the store's `CategoryBudgetCurrency.toBase`): on a cross-currency `convertSync` miss it returns `amount: 0` + `usedStaleFallback: true` — it SKIPS the tx rather than summing `tx.convertedAmount` (account currency) as base currency (CLAUDE.md ⚠️ #6, the `$20 + $100 = "120 KZT"` bug). `PreAggregatedData` gained an `fxStale` flag set when any cross-currency row was skipped during a cold-cache `build`; the self-heal is the M-9 trigger (CurrencyRatesNotifier.version → `invalidateAndRecompute()` wipes the cache and rebuilds with live rates). RED test `InsightsFXStaleResolverTests` pins: cold cross-currency miss → 0 (not convertedAmount), same-currency unaffected, warm cache converts, and `PreAggregatedData.build` skips the wrong-unit tx + flags stale. 325 unit tests green.
- **2026-05-25 — M-1 cross-currency inflow FIXED (decision: keep `principalDelta` separate).** Product confirmed multi-currency deposits exist (cross-currency top-ups via transfer). The `.internalTransfer` INFLOW leg of `principalDelta` now credits `targetAmount` (deposit currency) instead of source-side `convertedAmount` — so interest accrues on the correct principal for cross-currency top-ups (`DepositCrossCurrencyTransferTests`). `principalDelta` and `contribution` remain DELIBERATELY separate: the essential by-design divergence is the `.depositInterestAccrual` capitalization gate (principal compounds only when capitalization is on; balance always grows) — merging would corrupt non-capitalizing deposits. The outflow leg's `-amount` is correct (deposit is source → amount is in deposit currency). Also stabilized the pre-existing flaky `InsightsFXStaleResolverTests.warmCacheConverts` against the shared `CurrencyRateStore.shared` race. 343 unit tests green.
- **2026-05-25 — L-3 FIXED.** `LinkPaymentsView` nav-subtitle ("%d selected · %@") used raw `Formatting.formatCurrency` (always renders `.00`, CLAUDE.md red flag #7). Switched to `Formatting.formatCurrencySmart` (composed-string case → `String` formatter is the correct choice). UI-only; build green.
- **2026-05-25 — L-4 FIXED (doc-only).** `docs/domains/csv.md` listed 6 TransactionTypes but `CSVExporter.exportTypeName` writes 8. Doc now lists all 8 (`loan_payment`, `loan_early_repayment` added) and points at `exportTypeName` as the source of truth.
- **2026-05-25 — M-12 FIXED.** `MockAccountRepository` in `AccountRepositoryTests.swift` previously did `savedAccounts = accounts` wholesale on `saveAccounts`/`saveAccountsSync`, so it would clobber an existing account's balance — the opposite of the real `AccountRepository.saveAccountsInternal` UPDATE path which deliberately leaves `balance`/`initialBalance` untouched (owned by BalanceCoordinator). The mock now merges: existing accounts (by id) preserve persisted balance/initialBalance; new ids insert with incoming balance. Added `testSaveAccountsPreservesExistingBalance` (RED before fix) + `testSaveAccountsInsertsNewBalance`. Closes the false-confidence gap on the balance-clobber class.
- **2026-05-25 — M-5 FIXED.** `pauseSubscription` deleted the future transaction but left the matching future OCCURRENCE orphaned in `RecurringStore` (unlike `stopSeries`/`updateSeries`, which prune occurrences). The orphan made `resume` skip a period / gap: the generator's "should I generate?" guard saw an occurrence with no backing tx and declined to regenerate. Pause now mirrors `stopSeries` — `recurringStore.removeOccurrences(seriesId:afterDate: today)` + `saveOccurrences()` after deleting future txs. RED test `RecurringPauseOccurrenceTests` pins: (1) pause prunes the future occurrence (not just the tx); (2) resume regenerates exactly one correct future occurrence + tx. Both fail pre-fix, pass post-fix.
- **2026-05-25 — M-14 FIXED (account aggregates ordered after tx-save).** `saveAccountAggregates` was fire-and-forget `Task.detached`, so the import/rebuild flush returned before the aggregate CoreData write completed — a kill between the (re-throwing, ordered) tx-save and the detached aggregate write left warm-start aggregates lagging raw tx. Added an awaited `saveAccountAggregatesSync(...) async` (mirrors the existing `updateAccountBalancesSync` precedent; uses a UNIQUE per-call operation name so it never trips `CoreDataSaveCoordinator`'s duplicate-operation guard against an in-flight debounced save). `flushAccountAggregatePersist()` now routes through the sync variant; the fire-and-forget `saveAccountAggregates` just wraps the sync call in a detached Task. **Import hot path is NOT affected**: the flush runs ONCE at import/rebuild end (after `isImporting = false`, already inside `finishImport`'s `await`), never in the per-row loop; the debounced normal-CRUD persist still goes through the detached wrapper. Added to `DataRepositoryProtocol` + `AccountRepositoryProtocol`, forwarded in `CoreDataRepository`, stubbed in `UserDefaultsRepository` + `MockAccountRepository` (the three-places rule). Verified by build + full suite green; no unit test added for the kill-window itself (same rationale as H-10 — the substance is a control-flow ordering guarantee, not a behavior a unit test can exercise without a real CoreData stack injected into `AccountRepository`, which currently uses the `CoreDataStack.shared` singleton). **NOTE (follow-up):** the parallel category-aggregate persist (`CategoryRepository.saveAggregates`) has the identical fire-and-forget shape; a fully symmetric fix would add `saveAggregatesSync` and route `flushAggregatePersist` through it. Left as a noted follow-up to keep this change scoped to the audited item (M-14 = account aggregates).
- **2026-05-25 — M-13 FIXED (reconciliation point added).** `account.order` lives in `AccountOrderManager` (UserDefaults) while accounts live in CoreData, so the two stores can drift — a stale order key for a deleted account (the inline delete paths only call `removeOrder` when `!isImporting`, so an account deleted during import leaves an orphan key; a wholesale CSV account-set replacement is another source). Added `AccountOrderManager.reconcile(withAccountIds:)` which prunes order keys not present in the authoritative account set (no write when already in sync), called on load in `TransactionStore.loadData` right before `applyOrders`. Self-healing on next launch. Made `UserDefaults` injectable on `AccountOrderManager` (test seam; `.shared` still uses `.standard`). RED/contract test `AccountOrderManagerTests` pins prune / no-op / clear-all.
- **2026-05-25 — M-10 FIXED (downcast removed).** `BalanceCoordinator.persistBalance`/`persistBalances` downcast `repository as? CoreDataRepository` (logging a warning + no-op'ing the balance write on a miss) because the awaited `updateAccountBalancesSync` was missing from the `DataRepositoryProtocol` facade. The facade already had `updateAccountBalance`/`updateAccountBalances`, so the gap was narrow: promoted `updateAccountBalancesSync(...) async` to `DataRepositoryProtocol` (already on `CoreDataRepository`; added the no-op stub to `UserDefaultsRepository`; `MockAccountRepository` already had it). Both `persistBalance` and `persistBalances` now call `repository` directly through the facade — no downcast, no warning path. On the UserDefaults preview/fallback repo it's a no-op by design (M-11), which is the same behavior as before. Build + balance suites green.
- **2026-05-25 — M-7 VERIFIED resolved-by-H-7.** Both Insights amount resolvers (`resolveAmountStatic` and the instance `resolveAmount`) delegate to the single `resolveAmountToBase`, which on a cross-currency `convertSync` miss returns `0` + `usedStaleFallback: true` — it NEVER blends `tx.convertedAmount` (account currency) as base. Grep confirms no other `convertedAmount`-as-base path in `Services/Insights/`. Fixed a stale header doc-comment (line ~22) that still claimed "resolveAmount delegates to convertedAmount" (contradicted the H-7 code). M-7 = closed by H-7; no further code needed.
- **2026-05-25 — M-8 PARTIALLY-RESOLVED + residual DOCUMENTED (needs product decision).** Two projection sites in `InsightsService+CashFlow.swift`:
  - **Block 1 — `generateCashFlowInsights` "projected balance (30 days)" (~L114-151):** CLEAN. `projectedBalance = currentBalance (realized, future-EXCLUDED) + recurringNet`. `recurringNet`/`monthlyRecurringNet` is derived purely from active recurring SERIES definitions (frequency × amount, income vs expense by category type) — NOT from generated future-dated tx — so the single future occurrence excluded from `currentBalance` is counted exactly once (in the projection). No double-count.
  - **Block 2 — `generateCashFlowInsightsFromPeriodPoints` "projected balance" (~L240-271):** RESIDUAL double-count of RECURRING EXPENSES. `projectedNetChange = periodRecurringNet − periodAvgExpenses`, where `periodAvgExpenses` is the realized 3-period average of `periodPoints.expenses` (future-excluded post-H-5, so the future side is fine). But that historical average ALREADY contains the past charges of recurring EXPENSE series, while `periodRecurringNet` (negative for expense series) ADDS the projected recurring expense again → a recurring expense is subtracted twice. The future-inclusion bug the audit originally flagged is GONE (H-5 made `computePeriodDataPoints` realized-only); what remains is a *forecasting-model overlap*, not a future/realized mixing bug.
  - **Decision needed (do NOT guess):** define the forecasting model for Block 2 — should `avgMonthlyExpenses` EXCLUDE recurring-series expenses (so projection = realized balance + projected recurring + projected non-recurring average), or should Block 2 drop the separate `recurringNet` term and rely on the historical average alone (which already embeds recurring)? Either is a product/eng modeling call with edge cases (e.g. a series added this month has no history; a paused series still in the average). Left unchanged per the refactor's safety rule. (Block 1 is the user-facing "Projected balance" card on Home/Insights; Block 2 feeds the granularity-scoped formula detail.)
- **2026-05-25 — M-11 DOCUMENTED-ACCEPTED (preview/test-only).** `UserDefaultsRepository`'s balance updates (`updateAccountBalance`/`updateAccountBalances`/`updateAccountBalancesSync`) are all intentional no-ops. Verified production NEVER uses this path for balance persistence: `AppCoordinator.init` wires `repository ?? CoreDataRepository()` (always CoreData in the app); the `UserDefaultsRepository()` defaults on individual ViewModel inits are only the no-injection fallback (previews/tests). The one production `UserDefaultsRepository()` instantiation (`CoreDataRepository.clearAllData`) is a full-reset utility, not a balance path. Accepted as-is.
- **2026-05-25 — M-15 DOCUMENTED-ACCEPTED (round-trip verified symmetric).** CSV exports `convertedAmount` into the dual-purpose `targetCurrency`/`targetAmount` columns for non-transfer types (`CSVExporter` L74-90) and reads it back symmetrically on import (`EntityMappingService.convertRow` L246-251: `isTransfer = type == .internalTransfer` → non-transfer maps `targetAmount` column → `convertedAmount`, transfer → `targetAmount`). The `type` column disambiguates exactly as the doc's "targetCurrency / targetAmount Dual Purpose" table prescribes — round-trip preserves the value, no corruption. The audit's "hand-edited CSV skews base aggregation" concern is now moot: `convertedAmount` is in ACCOUNT currency and, post-H-7, base-currency aggregation uses `convertSync` (never `convertedAmount`-as-base), so a tampered `convertedAmount` cannot skew a base total. Contract already documented in `docs/domains/csv.md`. No fix needed.
- **2026-05-25 — L-1 DOCUMENTED-ACCEPTED (self-heals).** `lastTransactionDate` is intentionally NOT recomputed on the incremental REMOVE path (`TransactionStore+CategoryIndex.swift` L283-286 keeps the previous value to avoid an O(N) rescan per delete). It is display/sort-only and self-heals on the next cold rebuild. Accepted.
- **2026-05-25 — L-2 DOCUMENTED-ACCEPTED (masked by design).** `batchInsertTransactions` (NSBatchInsertRequest) sets the `recurringSeriesId` String column (L439) but intentionally omits the `recurringSeries` managed-object relationship (NSBatchInsertRequest bypasses the NSManagedObject lifecycle and cannot resolve relationships — documented inline L447-449). Harmless: `TransactionEntity.toTransaction()` reads `recurringSeriesId ?? recurringSeries?.id` (String first), so no reader depends on the relationship for batch-inserted rows. Accepted.
- **2026-05-25 — L-5 DOCUMENTED-ACCEPTED (no index needed).** `loadAllAccountBalances` is an unfiltered dictionary fetch of ALL `AccountEntity` rows (`propertiesToFetch = ["id","balance"]`, no predicate), not a per-`id` lookup — so there is no `WHERE id = ?` query that an index on `id` would accelerate. Trivial at ~50 accounts. Accepted.
- **2026-05-25 — M-3 INVESTIGATED → CONFIRMED can occur → DEFERRED (needs product decision; no half-measure).**
  - **Can a future-dated loan payment occur? YES.** Loans never auto-generate payments (loans.md §"No Auto-Reconciliation" — every payment is manual via `makeManualPayment` or a linked expense), so there is no recurring/future-occurrence path. BUT the manual payment/early-repayment forms (`LoanPaymentView`, `LoanEarlyRepaymentView`) pick the date via `dateButtonsSafeArea` → `DateButtonsDatePickerSheet`, whose `DatePicker` has **NO `in:` range constraint** (`DateButtonsView.swift` L106-110) — the graphical calendar lets the user select a FUTURE date. So a user can create a future-dated `.loanPayment` / `.loanEarlyRepayment`.
  - **The contradiction (precise):** `LoanPaymentService.createManualPayment` (L304) and `applyEarlyRepayment` (L174) mutate `loanInfo.remainingPrincipal -= …` SYNCHRONOUSLY at creation, regardless of the payment date. Since the loan account balance now DERIVES from `remainingPrincipal` (C-3), the debt drops NOW. Meanwhile the source-bank leg in `BalanceCalculationEngine.contribution` (L155-159) is gated by `affectsCurrentBalance` → `LedgerPolicyRule.isRealized` (future-EXCLUDED), so the bank balance does NOT decrease until the date matures. Net: between creation and the payment date, debt is reduced but the bank cash is not — net worth is temporarily wrong and the early debt reduction violates the realized-excludes-future policy. The day-change repair does NOT fix this: it recomputes the loan *balance* from `remainingPrincipal`, but `remainingPrincipal` was already (incorrectly) reduced early — the error is "too soon", not "too late".
  - **Decision (2026-05-25): chose option (A) — block future dates.** Loan payments record an *actual* payment you already made, so a future date is meaningless. Implemented two layers: (1) **VM guard** — `LoansViewModel.makeManualPayment` / `makeEarlyRepayment` now early-return `nil` (no `remainingPrincipal` mutation, no tx) when `!LedgerPolicyRule.isRealized(date)`; this protects every entry point including the bulk "pay all" flow (`payAllLoans` routes through `makeManualPayment`). (2) **UI constraint** — `dateButtonsSafeArea` and `DatePickerRow` gained an opt-in `maxDate` param (default nil = unchanged for all other forms, which legitimately allow future dates); `LoanPaymentView`, `LoanEarlyRepaymentView`, and `LoanPayAllView` pass `maxDate: Date()` so the calendar can't select a future day. RED→GREEN via `LoanFuturePaymentGuardTests` (future payment/early-repayment rejected & principal untouched; past payment accepted & principal reduced). Option (B) (deriving `remainingPrincipal` from realized tx) was NOT needed once future dates are disallowed at entry. 353 unit tests green. Also made `InsightsFXStaleResolverTests` `@MainActor` so it serializes on the main actor with the other `CurrencyRateStore.shared`-mutating suites (fixes the intermittent persistence-test flake).
- **Next:** all Critical + all High closed; M-1 resolved (cross-currency fixed; capitalization divergence kept by design). Remaining open items are Medium/Low only (see §2 tables: M-10..M-15, H-9, L-*), none user-facing-critical.
- **2026-05-25 — Flaky test FIXED.** `SubscriptionTransactionMatcherTests.findCandidates_matchesCrossCurrencyViaConvertedAmount` intermittently failed in the full run: it relies on `convertSync(USD→KZT)` returning nil (so the matcher falls back to `convertedAmount`), but a sibling `@MainActor` suite could repopulate the process-wide `CurrencyRateStore.shared` AFTER this suite's `init()` clear and BEFORE the matcher call. Added an extra `CurrencyRateStore.shared.clearAll()` at the top of the test body so the miss path is guaranteed regardless of inter-suite ordering (suite `init()` clear retained for general hygiene).
