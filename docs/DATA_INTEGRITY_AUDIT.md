...
- **Next (Phase 5 remainder — interrelated, dedicated session):** H-6 (Insights category totals are expense-only vs store all-types — same root as the C-6 residual; needs an expense-only budget/category read), H-7 (cold-FX `convertedAmount`-as-base fallback in `resolveAmount*` — tied to M-9), M-9 (single FX-version signal so late rate landings reconcile Insights + aggregates). Plus M-1 (deposit principalDelta), M-4 (recalculateAfterLinking actual amounts), H-10 (import partial-write atomicity).
