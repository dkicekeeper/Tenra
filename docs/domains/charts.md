...
**Insight mini-visuals & `Insight.cardVisual`** (2026-07 visual refresh): generators set a purpose-built `InsightCardVisual` (`barPair` / `halfGauge` / `ring` / `donut` / `budgetBars` / `milestoneGauge`) with ready-to-draw values; `InsightsCardView.miniChart` renders it and falls back to the legacy detailData-derived chart when nil. `cardVisual` is part of `Insight ==` (value-based equality gotcha). Shared visual grammar: **solid fill = fact, translucent/dashed = forecast (`barPair(isProjection:)`, `LinearProgressBar(projectedPercentage:)`), muted gray = comparison base, tick = norm/target marker (limit tick = destructive)**. `MiniBarPair` — pairwise was/now metrics (MoM, YoY, price increase, subscription growth, spending forecast). `MiniHalfGauge` — value vs norm tick on a RELATIVE scale `max(value × 1.15, norm × 2)` (spike, large tx, savings rate vs 20%); it is NOT a % -of-limit ring — that's `ProgressRing` (absolute scale, now with optional `showsTrack` for standalone contexts). `budgetBars` — up to 3 thin `LinearProgressBar`s (overspend shows top-3 offenders; projectedOverspend passes `projectedPercentage` → forecast mode: scale `max(projected, 110)`, translucent projection segment, destructive limit tick at 100%). `MiniMilestoneGauge` — discrete segmented scale with target tick (emergency fund months, 3 of 6). `.sparkline(points:series:projectedValue:markExtremes:)` — MiniSparkline with generator-shaped points (NO card-side tail slice): dashed projection tail + hollow endpoint (projectedBalance), min/max extreme dots over FULL history (period records), custom per-category points (categoryTrend). `MiniProportionBar` (`.proportionBar([DonutSlice])`) — stacked composition bar (recurring total: top-3 + other); flat bar, not a second donut, so adjacent feed cards stay visually distinct. All Canvas-based (no Apple Charts in feed cards), text-free (localization-free, `accessibilityHidden` — card text carries semantics).

**Hero visuals & the wow toolkit** (2026-07 visual refresh, detail screens): `InsightDetailView.heroChartSection` renders a full-size hero from the SAME `cardVisual` payload the feed mini draws — `HeroHalfGauge` (also the health-score hero, absolute 0–100 mode with `zoneTicks`), `HeroBarPair`, `HeroMilestoneGauge`, or a scaled-up `MiniSparkline`/`MiniProportionBar`. Shared effects live in [HeroChartEffects.swift](../../Tenra/Views/Components/Charts/HeroChartEffects.swift): `chartGlow()` (colour-matched blurred self-copy underlay — OrbChart's orbShadow generalised; static blur, follows trim/shape animations for free), `materialize(delay:)` (spring pop + sharpen-out-of-blur, Reduce-Motion aware), `glassBar(cornerRadius:)` (iOS 26 Liquid Glass, no-op fallback). Layer discipline: data = thin crisp elements, wow = soft underlays; NEVER animate a blur radius per-frame. Heroes are text-free (header/formula card carries numbers) — zero localization churn. `ring`/`budgetBars`/`donut` payloads deliberately have NO hero (their details already render full-size sections).
