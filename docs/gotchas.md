...
- **Staggered entrance animation** — stagger a multi-element reveal with per-view `.animation(anim.delay(index·step), value: trigger)` + one `@State` flip. NOT a loop mutating a shared `@State [T]` array (delays coalesce into one commit → all animate together) and NOT `DispatchQueue.asyncAfter` timers (re-entrancy/lifecycle risk). Fail-safe to visible (`!animatesOnAppear || reduceMotion`) so offscreen/headless renders aren't blank. Precedent: [`SpendingOrbChart.swift`](../Tenra/Views/Components/Charts/SpendingOrbChart.swift).
